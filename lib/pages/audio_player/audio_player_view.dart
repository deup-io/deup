...
class AudioPlayerPage extends GetView<AudioPlayerController> {
  ...
  Widget _buildPullDownButton() {
    ...
    if (controller.object.value.isLive != true && controller.file.isEmpty) {
      items.add(
        PullDownMenuItem(
          title: '添加到下载列表',
  ...
  /// 封面图
  Widget _buildCover() {
    final _blob = CommonUtils.getBlobData(controller.object.value.thumbnail);
    return Hero(
      tag: 'cover',
      child: ClipRRect(
        ...
        child: _blob != null
            ...
            : CachedNetworkImage(
                imageUrl:
                    controller.object.value.thumbnail ?? kDefaultLogoImageUrl,
                fit: BoxFit.cover,
