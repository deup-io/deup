...
extension JavascriptRuntimeXhrExtension on JavascriptRuntime {
  ...
  JavascriptRuntime enableXhr() {
    ...
    Timer.periodic(Duration(milliseconds: 40), (timer) async {
      ...
      int index = 0;
      while (index < pendingCalls.length) {
        XhrPendingCall pendingCall = pendingCalls[index] as XhrPendingCall;
        ...
        String responseText = '';
        try {
          responseText = hasUtf32Bom(response.bodyBytes)
              ? utf32.decode(response.bodyBytes)
              : (hasUtf16Bom(response.bodyBytes)
                  ? utf16.decode(response.bodyBytes)
                  : utf8.decode(response.bodyBytes));
        } catch (e) {
          responseText = gbk.decode(response.bodyBytes);
        }

        // backslashes will cause parsing errors, here needs to be escaped
        responseText =
            responseText.replaceAllMapped(RegExp(r'[\\`$]'), (match) {
